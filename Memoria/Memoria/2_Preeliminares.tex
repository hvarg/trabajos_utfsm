...
Esta característica es fundamental para la web semántica.
...
Podemos encontrar una descripción completa de Turtle en
~\cite{beckett2014turtle} y de RDF/XML en ~\cite{beckett2004rdf}.

El vocabulario incluido en la especificación RDF es básico y por ello fue
extendido a \emph{RDF Schema}.

En la figura \ref{fig:triples} podemos ver un ejemplo básico de la utilización 
de Turtle para la generación de triples y como estos pueden ser visualizados
como un grafo dirigido.

\begin{figure}[htpb]
  \centering
  \begin{tabular}{c}
  \lstinputlisting[basicstyle=\ttfamily\scriptsize]{Codigo/example-turtle.ttl}
  \end{tabular}
  %TODO Agregar tikz con el grafo.
  \caption{Ejemplo de un triple RDF.}
  \label{fig:triples}
\end{figure}
...
\tt{domain} y \tt{range} para el rango y dominio de cierta propiedad
