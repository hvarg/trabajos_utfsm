...
\begin{tikzpicture}[shorten >=1pt, node distance=3cm, on grid, >=stealth, 
	...
	\path[->]
		...
		(x3) edge 	node[above]{$H$}		node[below]{$(7,9)$}		(x5)
		(x5) edge 	node[below left]{$I$}	node[above right]{$(9,13)$}	(x7)
		(x6) edge 	node[above left]{$J$}	node[below right]{$(9,12)$}	(x7)
		(x7) edge 	node[below left]{$K$}	node[above right]{$(13,15)$}	(x9)
		(x7) edge 	node[above left]{$L$}	node[below right]{$(13,17)$}	(x8)
		(x9) edge 	node[above]{$M$}		node[below]{$(15,17)$}	(x10)
		(x8) edge 	node[above]{$N$}		node[below]{$(17,21)$}	(x11)
		(x10) edge	node[left]{$O$}			node[right]{$(17,22)$}	(x11)
		%%%   DUMMIES   %%%
		(x2) edge[dashed]	(x1)
		(x5) edge[dashed]	(x6)
		(x8) edge[dashed]	(x10)
